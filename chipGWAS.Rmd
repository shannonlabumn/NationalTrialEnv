...
programCodes <- data.frame(code = c("^A$", "^AA$", "^AW$", "^AC$", "^AO$",
                      ...
                      cross.site = c("Idaho", "Idaho", "Idaho", "Idaho", "Idaho", 
                                    ...
                                    "NorthDakota", "Wisconsin", "NorthCarolina", "Maryland"),
                       sel.site = c("Idaho", "Idaho", "Wisconsin", "Colorado", "Oregon", 
                                    ...
                                    "Oregon", "Wisconsin", "NorthCarolina", "Maryland"))
...
for(i in 1:9){
  for(j in 1:(9 - (i - 1))){
    store <- list(get.rho(matrix(as.numeric(as.matrix(list[[i]])), ncol = ncol(list[[i]]), 
                            dimnames = dimnames(list[[i]])), pops[[i]]))
...
```{r mega-environment}
# chipsMegaEnv <- chipPhenoFinal %>%
#   group_by(trial) %>%
#   filter(!duplicated(variety)) %>%
#   mutate(trial = as.character(trial)) %>%
#   statgenSTA::createTD(., genotype = "variety", trial = "trial") %>%
#   gxeMegaEnv(TD = ., trait = "yield_total") %>%
#   summary
# ## Summarize results.
# summary(chipsMegaEnv)
