...
chipPheno <- chipFieldData1 %>% 
              select(variety, yield_total, program, trial) %>%
              filter(!is.na(program), !is.na(yield_total))

write.csv(chipPheno, file = "../../Experimental/chipPheno.csv", row.names = F)
...
params <- set.params(geno.freq = 1 - 10/619)

data.loco.scan <- GWASpoly(data=data.loco,models=c("additive"),
                           traits=c("program"),params=params)
...
#SBATCH --error=gwaspoly.%J.stderr                                             
#SBATCH --output=gwaspoly.%J.stdout                                            
#SBATCH --mail-user=agha0012@umn.edu                                           
